...
export default function FanMap() {
  ...
  const zonesToRender = selectedStadium?.zones || fallbackZones;
  ...
  const handleZoneClick = async (zone) => {
    ...
    try {
      ...
      // We simulate asking the global voice/query endpoint for uniform translation/routing
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <Card className="h-[600px] relative bg-[#0f172a] border-gray-800 overflow-hidden flex items-center justify-center p-0">
            ...
            {zonesToRender.map((zone, i) => {
              // Calculate rough x, y if not provided (mock math)
              const x = zone.x || (20 + (i * 15) % 60);
              const y = zone.y || (20 + (i * 20) % 60);
              const color = zone.color || (zone.status === 'HIGH' ? 'bg-red-500' : zone.status === 'MEDIUM' ? 'bg-yellow-500' : 'bg-green-500');

              return (
              <button
                key={zone.id || i}
                onClick={() => handleZoneClick(zone)}
                className={`absolute transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-1 transition-transform hover:scale-110 ${activeZone?.name === zone.name ? 'scale-125 z-10' : 'z-0'}`}
                style={{ top: `${y}%`, left: `${x}%` }}
              >
                <div className={`w-6 h-6 rounded-full ${color} shadow-lg animate-pulse`}></div>
